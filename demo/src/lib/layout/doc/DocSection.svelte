		...
		{#each properties as { name, type, description, defaultValue }, i (name)}
			...
			<Section label={name} id="{titleLowercase}-{name}" level={3} headerClassName="text-primary-emphasis">
				<div class="d-flex flex-wrap gap-1 flex-column flex-md-row">
					<div class="flex-grow-1 mb-4" style:flex-basis={type.length > 40 ? '90%' : '45%'}>
						<div class="mb-1">
					...
					{#if defaultValue}
						<div class="flex-grow-1 mb-4" style:flex-basis="45%">
							<div class="mb-1"><span class="fw-bold">Default value:</span></div>
